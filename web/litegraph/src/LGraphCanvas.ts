...
export class LGraphCanvas implements CustomEventDispatcher<LGraphCanvasEventMap> {
  ...
  /**
   * Shortcuts are accepted only after the user has clicked inside {@link canvas}'s
   * parent (graph surface), or until they click outside — avoids stealing Ctrl+C etc.
   * from the app shell. Starts true so the editor route works before the first click.
   */
  private _graphKeyboardArmed = true;
  ...
  _pointerdown_scope_callback?(e: PointerEvent): void;
  ...
  constructor(
    ...
    console.log("constructor", canvas, graph, options);
    options ||= {};
  ...
  static onMenuNodeClone(
    ...
    const canvas = LGraphCanvas.active_canvas;
    const nodes = canvas.selectedItems.size
  ...
  bindEvents(): void {
    ...
    // Arm keyboard scope: pointerdown inside graph parent vs app chrome (navbar, etc.)
    this._pointerdown_scope_callback = (e: PointerEvent) => {
      const root = this.canvas.parentElement;
      const t = e.target as Node | null;
      if (!root || !t) return;
      if (root.contains(t)) {
        this._graphKeyboardArmed = true;
        LGraphCanvas.active_canvas = this;
      } else {
        this._graphKeyboardArmed = false;
      }
    };
    document.addEventListener(
      "pointerdown",
      this._pointerdown_scope_callback,
      true,
    );

    // Keyboard on document (capture): key events target the focused element, not the canvas.
    this._key_callback = (e: KeyboardEvent) => {
      console.log("key down", e.key);
      if (LGraphCanvas.active_canvas !== this || !this._graphKeyboardArmed) {
        return;
      }
      this.processKey(e);
    };
  ...
  unbindEvents(): void {
    ...
    this._pointerdown_scope_callback = undefined;
  ...
  /**
   * Editor shortcuts: copy/paste/duplicate/delete selection (Ctrl/Cmd+C/V/D, Delete, Backspace).
   * @returns true if the event was consumed
   */
  private _handleGraphKeyboardShortcuts(e: KeyboardEvent): boolean {
    const ctrlOrMeta = e.ctrlKey || e.metaKey;
    ...
    if (ctrlOrMeta && e.code === "KeyD") {
      if (!this.read_only && this.selectedItems.size > 0) {
        LGraphCanvas.cloneNodes([...this.selectedItems]);
        return true;
      }
      return false;
    }
    ...
    if (
      ...
      if (
        e.key === "Backspace" &&
        this.getCanvasWindow().document.activeElement === this.canvas
      ) {
        return true;
      }
      return false;
  ...
  processKey(e: KeyboardEvent): void {
    ...
    if (keyTarget instanceof HTMLElement) {
      const tag = keyTarget.localName?.toLowerCase();
      if (
        tag === "input" ||
        tag === "textarea" ||
        tag === "select" ||
    ...
    if (e.type == "keydown") {
      let graphShortcutHandled = false;
      // TODO: Switch
      if (e.key === " ") {
        // space
        this.read_only = true;
      ...
      } else if (this._handleGraphKeyboardShortcuts(e)) {
        LGraphCanvas.active_canvas = this;
        block_default = true;
        e.preventDefault();
        graphShortcutHandled = true;
      }

      if (!graphShortcutHandled) {
  ...
  drawFrontCanvas(): void {
    ...
    if (this.bgcanvas == this.canvas) {
      ...
      // Composite bg to the full front buffer. `resize()` uses layout/CSS pixel
      // dimensions; dividing by devicePixelRatio here only matched a physical-pixel
      // buffer convention and left the grid/pattern covering a fraction of the canvas.
      ctx.drawImage(this.bgcanvas, 0, 0, this.canvas.width, this.canvas.height);
    }
    ...
    if (graph) {
      ...
      this._drawDraggingSelectionRect(ctx);
  ...
  /** Marquee rectangle while drag-selecting nodes (graph space). */
  private _drawDraggingSelectionRect(ctx: CanvasRenderingContext2D): void {
    ...
    const lineWidth = 1 / this.ds.scale;
    const dash = 4 / this.ds.scale;
  ...
  drawBackCanvas(): void {
    ...
    // Same base transform as the front canvas: graph space via DragAndScale only.
    // An extra devicePixelRatio premultiply here made the grid / links drift vs nodes when zooming.
    ctx.setTransform(1, 0, 0, 1, 0, 0);
  ...
  createDefaultNodeForSlot(optPass: ICreateDefaultNodeOptions): boolean {
    ...
    if (!nodeX)
      throw new TypeError(
        "nodeX was null when creating default node for slot.",
      );
      ...
      const fUpdate: PanelWidgetCallback = (name, value) => {
        ...
        switch (name) {
          ...
          case "Mode": {
            if (typeof value !== "string")
              throw new TypeError(
                "Attempting to set mode to non-string value.",
              );
