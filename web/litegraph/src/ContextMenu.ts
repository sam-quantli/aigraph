...
export class ContextMenu<TValue = unknown> {
  ...
  constructor(
    ...
    if (options.event) {
      ...
      const margin = 10
      // Menu is position:fixed; clamp to the visual viewport, not document.body.
      // Body height can exceed innerHeight, which skipped clamping and left menus off-screen.
      const view = root_document.defaultView
      const vpW = view?.visualViewport?.width ?? view?.innerWidth ?? 0
      const vpH = view?.visualViewport?.height ?? view?.innerHeight ?? 0
      if (vpH === 0 && document.body.getBoundingClientRect().height === 0) {
        console.error(
      ...
      if (vpW > 0) {
        const maxLeft = vpW - root_rect.width - margin
        left = Math.min(Math.max(margin, left), Math.max(margin, maxLeft))
      }
      if (vpH > 0) {
        const maxTop = vpH - root_rect.height - margin
